...
# def save_preprocessing_report(df_original, df_cleaned, output_dir, filename):
#     """
#     Generate and save a preprocessing report
#     """
#     report_path = os.path.join(output_dir, f"{filename}_preprocessing_report.txt")
...
#         f.write(f"  Shape: {df_original.shape}\n")
#         f.write(f"  Columns: {list(df_original.columns)}\n")
#         f.write(f"  Missing values: {df_original.isnull().sum().sum()}\n\n")
        
#         f.write(f"Cleaned Dataset:\n")
#         f.write(f"  Shape: {df_cleaned.shape}\n")
#         f.write(f"  Columns: {list(df_cleaned.columns)}\n")
#         f.write(f"  Missing values: {df_cleaned.isnull().sum().sum()}\n\n")
        
#         # Feature summary for text classification
#         if 'label' in df_cleaned.columns:
#             f.write(f"Label Distribution:\n")
#             label_counts = df_cleaned['label'].value_counts().sort_index()
#             for label, count in label_counts.items():
#                 percentage = (count / len(df_cleaned)) * 100
#                 label_name = 'Spam' if label == 1 else 'Ham'
#                 f.write(f"  {label_name} ({label}): {count} ({percentage:.1f}%)\n")
#             f.write("\n")
        
#         # Feature statistics
#         if any(col in df_cleaned.columns for col in ['number_ratio', 'special_char_ratio', 'sus_words_count']):
#             f.write("Crucial Features Summary:\n")
#             crucial_features = ['number_ratio', 'special_char_ratio', 'sus_words_count']
#             for feature in crucial_features:
#                 if feature in df_cleaned.columns:
#                     mean_val = df_cleaned[feature].mean()
#                     std_val = df_cleaned[feature].std()
#                     f.write(f"  {feature}: μ={mean_val:.4f}, σ={std_val:.4f}\n")
